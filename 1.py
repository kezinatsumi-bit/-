...
def is_ball_in_arc(ball_pos, CIRCLE_CENTER, start_angle, end_angle):
    ...
    if start_angle <= ball_angle <= end_angle or (start_angle <= ball_angle + 2 * math.pi <= end_angle):
        return True
    return False
def draw_heart(window, center, size, color):
    #hình trái tim
    ...
    for t in np.linspace(0, 2 * np.pi, 100):
        #trai tim
        px = 16 * np.sin(t)**3
        py = 13 * np.cos(t) - 5 * np.cos(2*t) - 2 * np.cos(3*t) - np.cos(4*t)
...
while running:
	...
	for ball in balls:
		draw_heart(window, ball.pos, 8, ball.color) #chuyen thanh RED neu muon trai tim do
	pygame.display.flip()
	clock.tick(60)
pygame.quit()